...
internals.monitorOptions = Joi.object().keys({
    ...
    requestHeaders: [Joi.boolean(), Joi.array().items(Joi.string().valid('response', 'wreck'))],
    requestPayload: [Joi.boolean(), Joi.array().items(Joi.string().valid('response', 'wreck'))],
    responsePayload: [Joi.boolean(), Joi.array().items(Joi.string().valid('response', 'wreck'))],
