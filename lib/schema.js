...
internals.monitorOptions = Joi.object().keys({
    ...
    reqHeaders: Joi.array().items(Joi.string().valid('response', 'wreck')),
    resHeaders: Joi.array().items(Joi.string().valid('response', 'wreck')),
    reqPayload: Joi.array().items(Joi.string().valid('response')),
    resPayload: Joi.array().items(Joi.string().valid('response', 'wreck')),
