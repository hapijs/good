...
internals.NetworkMonitor.prototype.responseTimes = function (callback) {
    ...
    for (var i = 0, il = ports.length; i < il; ++i) {
        var port = ports[i];
        overview[port] = {
            avg: this._responseTimes[port].total / this._responseTimes[port].count,
...
internals.NetworkMonitor.prototype._onResponse = function (request) {
    ...
    this._responseTimes[port].total += request.msec;

    if (this._responseTimes[port].max < request.msec) {
        this._responseTimes[port].max = request.msec;
    }
};
