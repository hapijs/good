...
internals.Monitor.prototype._wreckHandler = function (error, request, response, start, uri) {
    ...
    if ((typeof options.responsePayload === 'boolean' && options.responsePayload) || (Array.isArray(options.responsePayload) && ~options.responsePayload.indexOf('wreck'))) {
        Wreck.read(response, {}, function (error, payload) {

            this._dataStream.push(Utils.GreatWreck(error, request, response, start, uri, options, payload));
        });
    }
    else {
        this._dataStream.push(Utils.GreatWreck(error, request, response, start, uri, options, null));
    }
};
