...
module.exports = internals.Monitor = function (plugin, options) {
    ...
    options.reporters = options.reporters || [{ reporter: GoodConsole }];
...
internals.Monitor.prototype.start = function (callback) {
    ...
    for (var i = 0, il = this.settings.reporters.length; i < il; i++) {
        ...
        if (item.reporter) {
            ...
            if (typeof item.reporter === 'string') {
                item.reporter = internals.requireReporter(item.reporter);
            }
    ...
    Async.each(this._reporters, function (item, next) {
        ...
        self._plugin.events.on(self.settings.responseEvent, self._state.responseHandler);
...
internals.Monitor.prototype.stop = function () {
    ...
    this._plugin.events.removeListener(this.settings.responseEvent, state.responseHandler);
...
internals.requireReporter = function (value) {

    if (value.toLowerCase() === 'good-console') {
        return GoodConsole;
    }
