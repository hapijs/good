...
class Monitor {
    ...
    start(callback) {
        ...
        internals.forOwn(this.settings.reporters, (reporterName, streamsSpec) => {
            ...
            const streamObjs = [];
            const pipeLength = streamsSpec.length;
            for (let i = 0; i < pipeLength; ++i) {
                const spec = streamsSpec[i];

                // Already created stream
                if (typeof spec.pipe === 'function') {
                    internals.validateStream(this.settings.strictPipeline, pipeLength - 1 , i, spec, reporterName, `The stream at position ${i}`);
                ...
                if (process[spec]) {
                    internals.validateStream(this.settings.strictPipeline, pipeLength - 1 , i, spec, reporterName, spec);
                ...
                internals.validateStream(this.settings.strictPipeline, pipeLength - 1 , i, stream, reporterName, moduleName);
...
internals.getStreamType = (stream) => {
...
internals.validateStream = (throwError, lastPos, pos, stream, reporterName, streamName) => {

    const type = internals.getStreamType(stream);

    let msg  = null;

    switch (type) {
        case 'transform':
            msg = pos !== lastPos ? null : `Error in ${reporterName}. ${streamName} is a transform stream and should not be the last stream in the pipeline.`;
            break;
        case 'writable':
            msg = pos === lastPos ? null : `Error in ${reporterName}. ${streamName} is a writable stream and should only be the last stream in the pipeline.`;
            break;
        case 'readable':
            msg = `Error in ${reporterName}. ${streamName} is a readable stream and should not be present in the pipeline.`;
            break;
    }

    if (msg) {
        if (throwError === true) {
            Hoek.assert(false, msg);
        }
        else {
            console.error(msg);
        }
