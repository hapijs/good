...
internals.Monitor.prototype._display = function (events) {
    ...
    for (var i = 0, il = events.length; i < il; ++i) {
        ...
        if (event.event === 'ops') {
	        ...
	        if (event.headers) {
		        try {
			        headers = ' request headers: ' + JSON.stringify(event.headers);
		        } catch (err) {
			        headers = ' request headers: ' + SafeStringify(event.headers);
		        }
	        }

	        if (event.requestPayload) {
		        try {
			        requestPayload = ' request payload: ' + JSON.stringify(event.requestPayload);
		        } catch (err) {
            ...
            internals.printEvent({
                timestamp: event.timestamp,
                tags: ['request'],
                data: event.instance + ': ' + method + ' ' + event.path + ' ' + query + statusCode + ' (' + event.responseTime + 'ms)' + headers + requestPayload + responsePayload
            });
        }
