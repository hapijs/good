...
class Monitor {
    constructor(server, options) {
        ...
        this._requestLogHandler = (request, event) => {
            ...
            this.push(() => new Utils.GreatRequest(request, event));
        };

        this._logHandler = (event) => {

            this.push(() => new Utils.GreatLog(event));
        };

        this._errorHandler = (request, error) => {

            this.push(() => new Utils.GreatError(request, error));
        };

        this._responseHandler = (request) => {

            this.push(() => new Utils.GreatResponse(request, this._responseOptions, this.settings.filter));
        };

        this._opsHandler = (results) => {

            this.push(() => new Utils.GreatOps(results));
        };

        this._wreckHandler = (error, request, response, start, uri) => {

            this.push(() => new Utils.GreatWreck(error, request, response, start, uri));
        };
            ...
            Items.serial(streamsSpec, (spec, done) => {
                ...
                if (typeof spec.pipe === 'function') {
                    ...
                    if (typeof spec.start === 'function') {
                        let start = [];
                        if (spec.start.args) {
                            start = start.concat(spec.start.args);
                        }
                        start.push(done);
                        return spec.start.apply(spec, start);
                    }
                    return done();
                }
                ...
                //TODO: Check for "special reporters" here and break early
                ...
                const startArgs = spec.start ? spec.start.slice(0) : [];
                if (typeof stream.start === 'function') {
                    startArgs.push(done);
                    return stream.start.apply(stream, startArgs);
                }
