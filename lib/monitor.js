...
internals.Monitor.prototype._broadcast = function () {
    ...
    return function () {
        ...
        var getFilteredEvents = function (destFilterTags, subscriberQueue) {
        ...
        Object.keys(self._subscriberQueues).forEach(function (dest) {
            ...
            var envelope = {
                ...
                timestamp: Date.now(),
                events: getFilteredEvents(self._subscriberTags[dest], subscriberQueue)
            };
            ...
            if (/^(http|https)\:/i.test(dest)) {
                Request({ method: 'post', uri: dest, json: envelope });                    // Ignore errors
                return;
            }

            self._logToFile(dest, envelope);
        });
    };
};
...
internals.Monitor.prototype._logToFile = function (folder, data) {

    var filePath = Path.join(folder, data.timestamp.toString());
    var stringified = JSON.stringify(data);
    Fs.writeFile(filePath, new Buffer(stringified));
};