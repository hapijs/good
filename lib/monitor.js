...
internals.Monitor.prototype._display = function (events) {
    ...
    for (var i = 0, il = events.length; i < il; ++i) {
        ...
        if (event.event === 'ops') {
            ...
            var statusCode = event.statusCode >= 400 ? event.statusCode.toString().red : 
                event.statusCode >= 300 ? event.statusCode.toString().cyan : event.statusCode.toString().green;
            Hoek.printEvent({
                timestamp: event.timestamp,
                tags: ['request'],
                data: event.instance + ': ' + event.method + ' ' + event.path + ' ' + query + ' ' + statusCode + ' (' + event.responseTime + 'ms)'
            });
        }
