...
module.exports = internals.Monitor = function (plugin, options) {
    ...
    if (Object.keys(this._eventQueues).length ||
        ...
        if (this._eventQueues.log) {
            ...
            process.on('uncaughtException', this._background.log);
        }
...
internals.Monitor.prototype.stop = function () {
    ...
    if (this._background.log) {
        process.removeListener('uncaughtException', this._background.log);
...
internals.Monitor.prototype._exception = function (err) {

    var event = {
        event: 'log',
        timestamp: Date.now(),
        tags: ['uncaughtException'],
        error: err,
        stack: err.stack,
        data: err.message
    };
