...
internals.Monitor.prototype._display = function (events) {
    ...
    for (var i = 0, il = events.length; i < il; ++i) {
        ...
        if (event.event === 'ops') {
            ...
            //var requestPayload = event.requestPayload ? ' request payload: ' + JSON.stringify(event.requestPayload) : '';
            var responsePayload = event.responsePayload ? ' response payload: ' + JSON.stringify(event.responsePayload) : '';
            ...
            internals.printEvent({
                timestamp: event.timestamp,
                tags: ['request'],
                //data: event.instance + ': ' + event.method + ' ' + event.path + ' ' + query + statusCode + ' (' + event.responseTime + 'ms)' + requestPayload + responsePayload
