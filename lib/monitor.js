...
function _getDestType(name, subscriptions) {
    ...
    else if (subscriptions.handler) {
        type = 'handler';
    }
    else if (/^(http|https)\:/i.test(name)) {
        type = 'http';
    }
...
module.exports = internals.Monitor = function (plugin, options) {
    ...
    for (var i = 0, il = subscriberKeys.length; i < il; ++i) {
        ...
        var destType = _getDestType(dest, subscriptions);
        ...
        this._subscriberHandlers[dest] = subscriptions.handler;
...
internals.Monitor.prototype._broadcastHandler = function () {

    var keys = Object.keys(this._subscriberQueues.handler);
    var keysLength = keys.length;

    for (var i = 0; i < keysLength; ++i) {

        var name = keys[i];
        var subscriberQueue = this._subscriberQueues.handler[name];
        var events = this._eventsFilter(this._subscriberTags[name], subscriberQueue);

        subscriberQueue.length = 0;                                     // Empty queue (must not set to [] or queue reference will change)
        this._callHandler(name, events);
    }
};
...
internals.Monitor.prototype._callHandler = function (name, events) {

    var handler = this._subscriberHandlers[name];
