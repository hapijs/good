...
class Monitor {
    ...
    start(callback) {
        ...
        internals.forOwn(this.settings.reporters, (reporterName, streamsSpec) => {
            ...
            const streamObjs = [];
            const lastIndex = streamsSpec.length - 1;
            for (let i = 0; i <= lastIndex; ++i) {
                const spec = streamsSpec[i];

                // Already created stream
                if (typeof spec.pipe === 'function') {
                    internals.validateStream(this.settings.strictPipeline, lastIndex , i, spec, reporterName, `The stream at position ${i}`);
                ...
                if (process[spec]) {
                    internals.assetOrLog(i === lastIndex, `Error in ${reporterName}. ${spec} is a writable stream and should only be the last stream in the pipeline.`, this.settings.strictPipeline);
                ...
                internals.validateStream(this.settings.strictPipeline, lastIndex , i, stream, reporterName, moduleName);
...
internals.validateStream = (throwError, lastIndex, index, stream, reporterName, streamName) => {

    if (stream instanceof Stream.Transform) {
        return internals.assetOrLog(index < lastIndex, `Error in ${reporterName}. ${streamName} is a transform stream and should not be the last stream in the pipeline.`, throwError);
    }
    ...
    if (stream instanceof Stream.Writable) {
        return internals.assetOrLog(index === lastIndex, `Error in ${reporterName}. ${streamName} is a writable stream and should only be the last stream in the pipeline.`, throwError);
    }

    if (stream instanceof Stream.Readable) {
        return internals.assetOrLog(false, `Error in ${reporterName}. ${streamName} is a readable stream and should not be present in the pipeline.`, throwError);
    }
};


internals.assetOrLog = (cond, msg, throwError) => {

    if (throwError === true) {
        Hoek.assert(cond, msg);
    }
    else if (!cond) {
        console.error(msg);
    }
