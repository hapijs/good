...
internals.Monitor.prototype._broadcastFile = function (callback) {

    var self = this;

    callback = callback || function (err) { };               // Swallow error

    Object.keys(this._subscriberQueues.file).forEach(function (file) {

        var subscriberQueue = self._subscriberQueues.file[file];
        var events = self._eventsFilter(self._subscriberTags[file], subscriberQueue);

        subscriberQueue.length = 0;                          // Empty queue (must not set to [] or queue reference will change)

        self._logToFile(file, events, callback);
    });
};
