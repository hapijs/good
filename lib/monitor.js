...
internals.Monitor.prototype._display = function (events) {
    ...
    for (var i = 0, il = events.length; i < il; ++i) {
        ...
        if (event.event === 'ops') {
            ...
            if (event.statusCode) {
                var color = 32;
                if (event.statusCode >= 500) color = 31
                else if (event.statusCode >= 400) color = 33
                else if (event.statusCode >= 300) color = 36;
                statusCode = '\x1b[' + color + 'm' + event.statusCode + '\x1b[0m';
            }

            var data = event.instance + ': ' + event.method + ' ' + event.path + ' ' + query;
            if (statusCode)
                data += ' ' + statusCode;
            data += ' (' + event.responseTime + 'ms)';
            Hoek.printEvent({
                timestamp: event.timestamp,
                tags: ['request'],
                data: data
