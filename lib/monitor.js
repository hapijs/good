...
module.exports = internals.Monitor = function (server, options) {
    ...
    this._includeOptions = {
        reqHeaders: this.settings.reqHeaders,
        resHeaders: this.settings.resHeaders,
        reqPayload: this.settings.reqPayload,
        resPayload: this.settings.resPayload
    };
...
internals.Monitor.prototype._responseHandler = function (request) {
    ...
    this._dataStream.push(Utils.GreatResponse(request, this._includeOptions, this.settings.filter));
};
...
internals.Monitor.prototype._wreckHandler = function (error, request, response, start, uri) {
    ...
    var self = this;
    var options = this._includeOptions;

    if (options.resPayload.indexOf('wreck') !== -1) {
        Wreck.read(response, {json: true}, function (error, payload) {

            self._dataStream.push(Utils.GreatWreck(error, request, response, start, uri, options, payload, self.settings.filter));
        });
    }
    else {
        self._dataStream.push(Utils.GreatWreck(error, request, response, start, uri, options, null, self.settings.filter));
    }
};
