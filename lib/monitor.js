...
module.exports = internals.Monitor = function (server, options) {
    ...
    this._server = server;
    this._dataStream = new Readable({ objectMode: true });
...
internals.Monitor.prototype._wreckHandler = function (error, request, response) {

    this._dataStream.push(Utils.GreatWreck(error, request, response));
};
