...
class Monitor {
    ...
    start(callback) {
        ...
        Async.eachSeries(Object.keys(this.settings.reporters), (reporterName, next) => {

            const streamsSpec = this.settings.reporters[reporterName];
            const streamObjs = [];

            Async.eachSeries(streamsSpec, (spec, done) => {

                // Already created stream
                if (typeof spec.pipe === 'function') {
                    streamObjs.push(spec);
                    if (typeof spec.stop === 'function') {
                        this._state.stops.push({
                            context: spec,
                            stop: spec.stop
                        });
                    }
    ...
    stop(callback) {
        ...
        Async.each(this._state.stops, (item, next) => {

            item.stop.call(item.context, (err) => {

                // Only log the error here, we don't want to prevent other stops because of one.
                if (err) {
                    console.error(err);
                }
                return next();
