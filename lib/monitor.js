...
module.exports = internals.Monitor = function (plugin, options) {
    ...
    // If they didn't provide any reporters, attach the default console one
    options.reporters = _reporters || [{ reporter: GoodConsole }];
...
internals.Monitor.prototype.start = function (callback) {
    ...
    for (var i = 0, il = this.settings.reporters.length; i < il; i++) {
        ...
        var _reporter = item.reporter ? new item.reporter(item.options) : item;

        this._reporters.push(_reporter);
    }
...
internals.Monitor.prototype.stop = function (callback) {
...
internals.Monitor.prototype._sendMessages = function (reporters) {

    Async.each(reporters, function (reporter, callback) {
...
internals.Monitor.prototype._logHandler = function (event) {
    ...
    this._sendMessages(this._reporters);
};
