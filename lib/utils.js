...
exports.GreatWreck = function (error, request, response) {

    if (this.constructor !== exports.GreatWreck) {
        return new exports.GreatWreck(error, request, response);
    }
...
internals.formatLogEntry = function (data) {

    var logEntry = Hoek.clone(data);
    logEntry.responseTime = new Date().getTime() - logEntry.request.time;

    // Attempt to parse request payload for readability
    var reqPayload = (logEntry && logEntry.request && logEntry.request.options) ? logEntry.request.options.payload : null;
    if (reqPayload) {
        try {
            var payload = JSON.parse(reqPayload);
            logEntry.request.options._payload = payload.payload ? payload.payload : payload;
        }
        catch (err) {
