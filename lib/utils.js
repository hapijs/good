...
exports.GreatResponse = function (request, options, filterRules) {
    ...
    if (options.requestHeaders.indexOf('response') !== -1) {
        this.headers = req.headers;
    }

    if (options.requestPayload.indexOf('response') !== -1) {
        this.requestPayload = request.payload;
    }

    if (options.responseHeaders.indexOf('response') !== -1 && request.response) {
        this.responseHeaders = request.response.headers;
    }

    if (options.responsePayload.indexOf('response') !== -1 && request.response) {
...
exports.GreatWreck = function (error, request, response, start, uri, options, payload, filterRules) {
    ...
    if (options.requestHeaders.indexOf('wreck') !== -1) {
    ...
    if (options.responseHeaders.indexOf('wreck') !== -1) {
