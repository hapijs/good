...
exports.GreatResponse = function (request, options, filterRules) {
    ...
    if (options.reqHeaders.indexOf('response') !== -1) {
        this.headers = req.headers;
    }

    if (options.reqPayload.indexOf('response') !== -1) {
        this.reqPayload = request.payload;
    }

    if (options.resHeaders.indexOf('response') !== -1 && request.response) {
        this.resHeaders = request.response.headers;
    }

    if (options.resPayload.indexOf('response') !== -1 && request.response) {
        this.resPayload = request.response.source;
    }

    if (Object.keys(filterRules).length) {

        internals.applyFilter(this.reqPayload, filterRules);
        internals.applyFilter(this.resPayload, filterRules);
    }
...
exports.GreatWreck = function (error, request, response, start, uri, options, payload, filterRules) {
    ...
    if (options.reqHeaders.indexOf('wreck') !== -1) {
    ...
    if (options.resHeaders.indexOf('wreck') !== -1) {
