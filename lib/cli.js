...
internals.broadcast = function (log, retries, wait) {
    ...
    internals.request(JSON.stringify(envelope), retries, wait);
};


internals.request = function (payload, retryNumber, wait) {
    ...
    var errorHandler = (function (r) {
        ...
        return function(err) {
            ...
            console.error(err);
            console.info('Error uploading logs. Retrying in %s milliseconds...', wait);

            // Try again if there was an error
            setTimeout(function () {

                r -= 1;
                internals.request(payload, r, wait);
            }, wait);
        };
    }(retryNumber));
        ...
        Fs.exists(config.path, function (exists) {
            ...
            Fs.stat(config.path, function (err, stat) {
                ...
                internals.getLog(config.path, start, function (bytesRead, log) {
                    ...
                    start += bytesRead;
                    internals.broadcast(log, config.retries, config.wait);
                    if (config.useLastIndex) {
