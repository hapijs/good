...
- `[reqHeaders]` - determines if all request headers will be available to `reporter` objects. Defaults to `[]`. Valid array values are `wreck` and/or `response`. The request headers will be included in the events specified within the array.
- `[resHeaders]` - determines if all response headers will be available to `reporter` objects. Defaults to `[]`. Valid array values are `wreck` and/or `response`. The response headers will be included in the events specified within the array.
- `[reqPayload]` - determines if the request payload will be available to `reporter` objects. Defaults to `[]`. Valid array values are `response`. The request payload will be included in the events specified within the array.
- `[resPayload]` - determines if the response payload will be available to `reporter` objects. Defaults to `[]`. Valid array values are `wreck` and/or `response`. The response payload will be included in the events specified within the array.
...
- `reqPayload` - maps to `request.payload`.
- `resPayload` - maps to `request.response.source`.
